...
public class ClientDto {
    ...
    @NotBlank(message = "{client.error.name.blank}")
    @Size(min = 3,max = 40, message = "{client.error.name.size}")
    private String name;

    @NotBlank(message = "{client.error.phone.blank}")
    @Pattern(regexp = "\\d{10}",message = "{client.error.phone.pattern}")
